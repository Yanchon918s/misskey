...
<button
	...
	class="_button"
	:class="[$style.root, { [$style.reacted]: myReaction == targetReaction, [$style.canToggle]: canInteract, [$style.small]: prefer.s.reactionsDisplaySize === 'small', [$style.large]: prefer.s.reactionsDisplaySize === 'large' }]"
	@click="onClick"
		...
		misskeyApi('notes/reactions/delete', {
			...
			if (oldReaction !== reaction) {
				misskeyApi('notes/reactions/create', {
					...
					if (emoji == null && getUnicodeEmojiOrNull(reaction) == null) return;
...
async function menu(ev: PointerEvent) {
	...
	if (canToggle.value) {
		menuItems.push({
			...
			icon: 'ti ti-palette',
			action: () => {
				addToEmojiPalette(remoteReaction.value != null ? `:${remoteReaction.value.name}:` : isLocalCustomEmoji ? `:${emojiName.value}:` : props.reaction);
			},
		});
	}
